 ...
 * @param {Object} locations.members - the members at a location
...
class MongoDb {
  ...
  async deletePlant(id, userIdParam, logger) {
    ...
    try {
      ...
      /**
       * @typedef NoteSplit
       * @property {String[]} singlePlantNotes
      ...
      if (multiplePlantNotes.length > 0) {
        const updatedNotes = multiplePlantNotes.map(note => Object.assign(
          {},
          note,
          { plantIds: note.plantIds.filter(plantId => plantId !== id) },
        ));
