...
class MongoDb {
  ...
  async GetDb() {
    try {
      ...
      localLogger.trace({ moduleName, msg: 'mongoConnection', connection });
      if (!this.db) {
  ...
  async _close() {
    localLogger.trace({ moduleName, msg: 'Closing DB Connection.' });
    try {
      const db = await this.GetDb();
      await db.close(true);
    } catch (closeErr) {
      localLogger.error({ moduleName, msg: 'Error calling db.close()', closeErr });
    }
  }
  ...
  async findOrCreateUser(userDetails) {
    try {
      ...
      if (user && user.length !== 1) {
        localLogger.error({ moduleName, msg: `Unexpected user.length: ${user.length}`, user });
      }
      ...
      localLogger.error({ moduleName, msg: 'findOrCreateUser Error', error });
      throw error;
    }
  }
  ...
  async getUserNameByIds(userIds) {
    try {
      ...
      if (users.length < 1) {
        localLogger.error({
          moduleName,
          msg: `Unexpected number of users users.length: ${users.length}`,
          users,
        });
      }
  ...
  async getUserById(userId) {
    try {
      ...
      localLogger.warn({ moduleName, msg: 'No user found in query', userQuery });
      return undefined; // for lint
    } catch (err) {
      localLogger.error({ moduleName, msg: 'getUserById', userId });
      throw err;
    }
  }
  ...
  async createPlant(plant, loggedInUserId) {
    try {
      const db = await this.GetDb();
      if (!plant.userId) {
        localLogger.warn({ moduleName, msg: 'Missing plant.userId', plant });
  ...
  async getPlantById(plantId, loggedInUserId) {
    ...
    try {
      ...
      localLogger.warn({ moduleName, msg: 'No plant found in query', query });
      return undefined;
    } catch (error) {
      localLogger.error({
        moduleName,
  ...
  async getPlantsByIds(ids, loggedInUserId) {
    try {
      ...
      localLogger.error({ moduleName, msg: 'getPlantsByIds', error });
      throw error;
    }
  }
  ...
  async createNote(note) {
    try {
      ...
      localLogger.trace({ moduleName, msg: 'createdNote', createdNote });
  ...
  async getNotesByQuery(query) {
    try {
      ...
      localLogger.trace({ moduleName, msg: 'getNotesByQuery nothing found', query });
      return undefined;
    } catch (error) {
      localLogger.error({
  ...
  async getNoteByQuery(query) {
    try {
      ...
      localLogger.error({ moduleName, msg: 'getNoteByQuery', query });
      throw error;
    }
  }
  ...
  getNotesByPlantId(plantId) {
    const query = { plantIds: new ObjectID(plantId) };
    return this.getNotesByQuery(query);
  }

  getNotesByPlantIds(plantIds) {
    const query = {
      plantIds: {
  ...
  async createLocation(loc) {
    const db = await this.GetDb();
    return modelLocation.createLocation({ db, loc, logger: localLogger });
  }
  ...
  async getAllUsers() {
    try {
      ...
      localLogger.error({ moduleName, msg: 'getAllUsers', error });
      throw error;
    }
  }
  ...
  async getLocationsByQuery(query, options) {
    ...
    return modelLocation.getLocationsByQuery({
      db, query, options, logger: localLogger,
    });
  }
  ...
  async roleAtLocation(locationId, userId, roles) {
    ...
    return modelLocation.roleAtLocation({
      db, locationId, userId, roles, logger: localLogger,
    });
  }
  ...
  async updateLocationById(location, loggedInUserId) {
    ...
    return modelLocation.updateLocationById({
      db, location, loggedInUserId, logger: localLogger,
    });
  }
  ...
  The MongoDB driver requires an atomic operation on update.
   ...
   async replaceDocument(collection, doc) {
    try {
      ...
      localLogger.error({moduleName, msg: 'replaceDocument', error, collection, doc });
      throw error;
    }
  }
*/
