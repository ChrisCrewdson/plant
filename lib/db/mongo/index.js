...
class MongoDb {
  ...
  async GetDb() {
    ...
    try {
      if (!this.db) {
        this.db = await MongoClient.connect(mongoConnection);
      }
      return this.db;
    } catch (err) {
  ...
  async findOrCreateUser(userDetails) {
    try {
      ...
      if (user && user.length > 0) {
        // eslint-disable-next-line no-param-reassign
        user = this.convertIdToString(user[0]);
      }
  ...
  async getUserById(userId) {
    try {
      ...
      if (users) {
        ...
        const db = await this.GetDb();
        const locations = await read(db, 'location', locationQuery, { _id: true, members: true }, {});
  ...
  async createLocation(loc) {
    try {
      ...
      return this.convertLocationDataForRead(createdLocation);
    } catch (error) {
  ...
  async getAllLocations() {
    return this.getLocationsByQuery({}, {});
  }

  async getLocationById(id) {
    const _id = new ObjectID(id);
    return this.getLocationsByQuery({ _id }, {});
  }
  ...
  async getLocationOnlyById(id) {
    const _id = new ObjectID(id);
    const location = await this.getLocationsByQuery({ _id }, { locationsOnly: true });
  ...
  async getLocationsByIds(ids) {
    ...
    return this.getLocationsByQuery(query, {});
  }
  ...
  async roleAtLocation(locationId, userId, roles) {
    try {
      ...
      const location = await this.getLocationOnlyById(locationId);

      const role = location && location.members && location.members[userId];
      return roles.includes(role);
    } catch (error) {
      logger.error('roleAtLocation', { error, locationId, userId, roles });
      return false;
    }
  }
