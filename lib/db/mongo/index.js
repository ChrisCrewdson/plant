 ...
 * @param {BizUser[]} users - an array of user object
 * @param {DbLocation[]} locations - an array of location objects
...
function getUsersWithLocations(users, locations) {
  return (users || []).map((user) => {
    /**
     * @type {Array<import('mongodb').ObjectID>}
    */
    const init = [];
...
class MongoDb {
   ...
   * @param {object} userDetails - the object that Facebook/Google OAuth returns
  ...
  async findOrCreateUser(userDetails, logger) {
    try {
      ...
      const userFromSocialMedia = async () => {
         ...
         * @type {DbUser[]}
         */
        const user = await read(db, 'user', queryBySocialMedia, {});
      ...
      const userFromEmail = async () => {
         ...
         * @type {DbUser[]}
         */
        const dbUser = await read(db, 'user', queryByEmail, {});
       ...
       * @type {DbUser}
       */
      const dbUser = await Create.createOne(db, 'user', userDetails);
      /**
       * @type {BizUser}
       */
      const newUser = dbHelper.convertIdToString(dbUser);
      /**
       * @type {BizLocation}
       */
      const location = {
   ...
   * @param {Object|Array} plant
   * @param {String|null} loggedInUserId
   * @param {Logger} logger
   * @returns {Promise}
   */
   ...
   * @param {String} loggedInUserId - the id of the user currently logged in or falsy if
   *   anonymous request
   * @param {Logger} logger
   ...
   * @param {DbNote|DbNote[]} note
   * @param {Logger} logger
   * @returns {BizNote|BizNote[]}
   * @memberof MongoDb
   */
  ...
  async getNotesWithPlants(notesCopy, logger) {
    try {
      ...
      */
      ...
      const myPlantIds = notesCopy.reduce((acc, { plantIds }) => acc.concat(plantIds), init);
  ...
  async getAllUsers(logger) {
    try {
       ...
       * @type {DbUser[]}
       */
      const dbUsers = await read(db, 'user', userQuery, userOptions);
      /**
       * @type {BizUser[]}
       */
