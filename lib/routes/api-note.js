...
module.exports = (app) => {
  ...
  app.post('/api/note', requireToken, (req, res) => {
    ...
    validateNote(note, (validateNoteError, transformed) => {
      ...
      return mongo.upsertNote(transformed)
        .then(() => res.status(200).send({ success: true, note: transformed }))
  ...
  app.post('/api/notes', (req, res) => {
    ...
    if (plantId) {
      return mongo.getNotesByPlantId(plantId)
        .then(okay)
        .catch(() => res.status(500).send('Error getting notes by plantId'));
    }

    return mongo.getNotesByIds(noteIds)
      .then(okay)
      .catch(() => res.status(500).send('Error getting notes by noteIds'));
  });
  ...
  */
  ...
  function imageNote(req, res) {
    ...
    // 2. Push files to S3
      ...
      const updateNoteAndUploadImages = async () => {
      ...
      return updateNoteAndUploadImages();
    } catch (upsertNoteError) {
  ...
  app.put('/api/image-complete', (req, res) => {
    ...
    return mongo.addSizesToNoteImage(noteUpdate)
      .then(() => res.send({ success: true }))
      .catch(() =>
        // Error has already been logged in data layer
        res.status(500).send({ success: false, message: 'Error updating note' }));
  });
};
