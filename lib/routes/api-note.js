...
module.exports = (app) => {
  ...
  app.post('/api/note', requireToken, async (req, res) => {
    ...
    try {
      transformed.userId = req.user && req.user._id;
   ...
   * @param {NoteImage} image - an image object
   ...
   * @param {Array<NoteImage>} images - collection of images
   */
   ...
   * @param {String} options.data.userid
   * @param {String} options.data.noteid
  ...
  async function imageNote(req, res) {
    ...
    //   ]}
      ...
      note.userId = req.user && req.user._id;
      const noteid = note._id.toString();

      const files = (req.files || []).map(createFileFromMulterObject);
