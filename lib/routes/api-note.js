...
module.exports = (app) => {
  ...
  app.post('/api/note', requireToken, async (req, res) => {
    const { logger } = req;
    logger.trace({
      moduleName, msg: 'upsertNote()', 'req.body': req.body, method: req.method,
    });
  ...
  app.post('/api/notes', async (req, res) => {
    const { logger } = req;
    logger.trace({ moduleName, msg: '/api/notes body', body: req.body });
    ...
    if (plantIds) {
      try {
        const notes = await mongo.getNotesByPlantIds(plantIds);
        return okay(notes);
      } catch (e) {
  ...
  */
  ...
  async function uploadImages({ data, logger }) {
    ...
    const promises = data.files.map((file) => {
      ...
      logger.trace({
        moduleName, msg: 'About to upload to s3', params, Metadata,
      });
  ...
  async function imageNote(req, res) {
    ...
    // 2. Push files to S3
      logger.trace({
        moduleName, msg: 'imageNote', 'req.body': req.body, 'req.files': req.files,
      });
      if (!req.body.note) {
