...
function auth(app, passport) {
  ...
  app.get('/auth/dev', async (req, res) => {
    ...
    // find the user in the database based on their facebook id
      const user = await mongoDb.findOrCreateUser(newUser);
      if (!user) {
    ...
    try {
      passport.authenticate(`${strategy}`, (passportAuthenticateError, user /* , info */) => {
        if (passportAuthenticateError) {
          logger.error(`app.get(/auth/${strategy}/callback) error`, { passportAuthenticateError, user });
      ...
      logger.error(`Passport.authenticate ${strategy} threw error`, { passportAuthenticateError });
