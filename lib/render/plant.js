...
const target = async (req, res) => {
  ...
  try {
    ...
    if (user.locationIds && user.locationIds.length && !user.activeLocationId) {
      ...
      [user.activeLocationId] = user.locationIds;
    }
    ...
    return res.status(404).send(indexHtml({ req }));
  }
};
